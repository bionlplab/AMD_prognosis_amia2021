...
def evaluate_AUCs(model, x_dev, label_fit, labelv, losses, print_each_k_means = True):
    '''
        Evaluates for deep learning models:
            test loss
    ...
    '''
    outputv = model.predict(x_dev)
    outputv = outputv.flatten()

    event_occurv = []
    time_distv = []
    for i in range(len(labelv[0])):
        time_distv.append(labelv[0][i])
        event_occurv.append(labelv[1][i])
    event_occurv = np.array(event_occurv)
    time_distv = np.array(time_distv)

    idxv = pandas.Series(time_distv).sort_values(ascending=False).index
    ...
    for i in range(len(label_fit[0])):
        lab_train.append((bool(label_fit[1][i]), int(label_fit[0][i])))
    for i in range(len(labelv[0])):
        lab_val.append((bool(labelv[1][i]), int(labelv[0][i])))
